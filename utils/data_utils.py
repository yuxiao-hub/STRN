...
def list_cmu_sequence_files(path_to_dataset, action):
    """Return only numbered CMU source files in deterministic numeric order."""
    action_dir = os.path.join(path_to_dataset, action)
    if not os.path.isdir(action_dir):
        raise FileNotFoundError("CMU action directory not found: {}".format(action_dir))

    pattern = re.compile(r"^{}_([0-9]+)\.txt$".format(re.escape(action)))
    numbered_files = []
    for name in os.listdir(action_dir):
        match = pattern.match(name)
        if match is not None:
            numbered_files.append((int(match.group(1)), name))

    numbered_files.sort(key=lambda item: item[0])
    if not numbered_files:
        raise FileNotFoundError(
            "No numbered .txt source files found for CMU action '{}' in {}".format(
                action, action_dir
            )
        )

    expected = list(range(1, len(numbered_files) + 1))
    actual = [number for number, _ in numbered_files]
    if actual != expected:
        raise RuntimeError(
            "CMU source files for '{}' must be consecutively numbered from 1; "
            "found {}".format(action, actual)
        )

    return [os.path.join(action_dir, name) for _, name in numbered_files]
...
def load_data_cmu_3d_n(opt, path_to_dataset, actions, input_n, output_n, is_test=False):
    ...
    for action_idx in np.arange(nactions):
        ...
        sequence_files = list_cmu_sequence_files(path_to_dataset, action)
...
def load_data_cmu_3d_all(path_to_dataset, actions, input_n, output_n):
    seq_len = input_n + output_n
    nactions = len(actions)
    sampled_seq = []
    complete_seq = []
