...
def main(opt):
    ...
    if not opt.is_eval:
        ...
        train_data_loader = DataLoader(train_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=0, pin_memory=True)
    ...
    if not opt.is_eval:
        ...
        for epo in range(start_epoch, opt.epoch + 1):
            ...
            ret_train = run_model(net_pred, optimizer, is_train=0, data_loader=train_data_loader, epo=epo, opt=opt)
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        # p3d_h36 Tensor (32, 60, 96)
        batch_size, seq_n, _ = p3d_h36.shape
        # batch_size 32  seq_n 60 _ 96
        ...
        # p3d_sup (ground_truth) :Tensor(32, 20, 22, 3)
        p3d_src = p3d_h36.clone()[:, :, dim_used]
        # p3d_src :Tensor(32, 60, 66)
        p3d_out_all = net_pred(p3d_src, input_n=in_n, output_n=out_n, itera=itera)
        # p3d_out_all Tensor(32, 20, 1, 66)
        ...
        if is_train == 0:
            ...
            p3d_all_acc = p3d_all_smooth[:, 1:, :, :] - p3d_all_smooth[:, :-1, :, :]
            p3d_sup_acc = p3d_sup_smooth[:, 1:, :, :] - p3d_sup_smooth[:, :-1, :, :]
            loss_acc = torch.mean(torch.norm(p3d_all_acc - p3d_sup_acc, dim=3))

            loss_all = a * loss_p3d + b * loss_vel + c * loss_acc
            optimizer.zero_grad()
            loss_all.backward(retain_graph=True)
            # loss_all.backward()
            nn.utils.clip_grad_norm_(list(net_pred.parameters()), max_norm=10000)
            optimizer.step()
            # update log values
